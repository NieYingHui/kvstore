...
int main() {
	...
	char* keyArray[10] = {"key5","key3","key8","key1","key4", "key7","key9","key2","key6","key0"};
	char* valueArray[10] = {"value5","value3","value8","value1","value4", "value7","value9","value2","value6","value0"};
	...
	for (i = 0;i < 10;i ++) {
		...
		node->key = malloc(strlen(keyArray[i]) + 1);  // 这里需要重新修改
...
int kvs_rbtree_set(kvs_rbtree_t *inst, char *key, char *value){
	...
	/* 先查重：原 rbtree_insert() 遇到重复 key 会直接 return，
	 * 但 kvs_rbtree_set() 之前仍会分配 node 导致泄漏，且无法向上层表达“已存在”。
	 */
...
char* kvs_rbtree_get(kvs_rbtree_t *inst, char *key){
	...
	if(node == inst->nil || !node) return NULL;
...
int kvs_rbtree_del(kvs_rbtree_t *inst, char *key){
	...
	rbtree_node * node = rbtree_search(inst, key);
	if(node == inst->nil || !node) return 1; // not exist
...
int kvs_rbtree_exist(kvs_rbtree_t *inst, char *key){
	...
	if(!node || node == inst->nil) return 1; // not exist
