...
//Connection 
// 'C' + 'o' + 'n'
static int _hash(char *key, int size) { // 是一个静态函数（static），表示它只能在当前文件内被访问。
	...
	return sum % size;
// 哈希函数主要用于哈希表实现中，将字符串键转换为哈希表中的索引位置。
// 通过这种方式，可以快速定位数据在哈希表中的存储位置，从而实现高效的插入、查找和删除操作。
}
...
void kvs_hash_destory(kvs_hash_t *hash) {
	...
	for (i = 0;i < hash->max_slots;i ++) {
        ...
        hash->nodes[i] = NULL;  // 将槽位置空，但这是可选的
	}
